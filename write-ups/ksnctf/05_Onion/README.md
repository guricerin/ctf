謎の文字列をローカルのファイルにコピペしておく。ここでは``onion.txt``。  
ぱっと見で英数字しか使われてなさそうなのでbase64デコードしてみたらビンゴ。  

```bash
$ cat onion.txt | sed -z 's/\n//g' | base64 -d
...
問題タイトルからして、ひたすらbase64デコードしつづけるのだろうと推測。面倒なのでスクリプトでエラーが出るまでデコードさせる。  

```bash
$ ./dec.sh
...
WW1WbmFXNGdOalkySUR4a1lYUmhQZ28xTVVSUklURlZYU1k1TkZGSE5DTXRNem8wSlRjNU4wazNO Q1JCVlFvZ0NtVnVaQW89Cg==
YmVnaW4gNjY2IDxkYXRhPgo1MURRITFVXSY5NFFHNCMtMzo0JTc5N0k3NCRBVQogCmVuZAo=
begin 666 <data> 51DQ!1U]&94QG4#-3:4%797I74$AU end
base64: invalid input
```

なにやらそれっぽい文字列が出てきたが、``51DQ!1U]&94QG4#-3:4%797I74$AU``が答えというわけでもないらしい。  
念のため一段前の文字列をデコード。  

```bash
$ echo "YmVnaW4gNjY2IDxkYXRhPgo1MURRITFVXSY5NFFHNCMtMzo0JTc5N0k3NCRBVQogCmVuZAo=" | base64 -d
begin 666 <data>
...
``begin 666``でググったら``uuencode``なるものが出てきた。  
仕組みはよくわからんが、http://www.motohasi.net/Misc/UUConv.php に``51DQ!1U]&94QG4#-3:4%797I74$AU``をぶちこんでフラグ入手。  

### 参考

- [uuencode - Wikipedia](https://ja.wikipedia.org/wiki/Uuencode)
- [uuencode方式エンコード、デコード変換](http://www.motohasi.net/Misc/UUConv.php)
